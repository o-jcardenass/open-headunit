...
|---|---|---|---|---|
| Candidate | `fix/audio-sink-and-wireless-bring-up` | `f2e8d5bf` | 6 on `main` | 1974 tests, 0 failures |
| Baseline | `main` | `8418ed02` | | |

```bash
git fetch fork
git checkout -B audio-sink fork/fix/audio-sink-and-wireless-bring-up   # f2e8d5bf
git log --oneline -6
# f2e8d5bf Old Android
# 5bf505d7 Projection
# ed1b74b8 Connection
# 61143283 Native AA
# 762d195c Transport
...
**Check the branch out fresh; a round 2 checkout does not fast-forward to it.** This brief was
written against a branch that has since been renamed, consolidated, folded once and added to twice,
so the short version is the only version worth carrying: everything unlanded of ours is now this one
branch, one commit per component, and `fix/audio-sink-jitter-and-instrumentation`,
`fix/native-aa-double-group-create` and `fix/wireless-only-when-selected` are deleted locally and on
`fork`, along with every SHA rounds 1 and 2 cite. **The Audio and Transport work those rounds graded
is unchanged in content**, checked by tree comparison; the other four commits are what is new, and
section 3 says what each can do to a run. All six compile alone, checked.
...
**The Connection commit gained a fifth refusal after this brief was written, and it can make Part P
quieter.** The driver selector used to arm the wireless stack by itself: a unit with two phones
paired opened it at app start, its ten second countdown picked one with nobody touching it, and the
poke that followed claimed to be a user request, which is the one thing that gets past the
connection-mode gate. It is gated now. A unit whose selection includes WiFi is unaffected, so
**report `connectionModes` for both units** and confirm the group is up before grading anything. The
line that says it fired is
`HomeFragment: driver selection skipped, wireless is not a chosen connection mode`.

**The Projection commit is new to this candidate and changes what is on screen.** The performance
overlay moved into its own file and now draws on units where nothing lifted it before, `Temp:` reads
a figure where one unreadable thermal zone used to blank the whole scan, and the app CPU figure is a
share of the whole device rather than of one core, so it can no longer read past the system figure
beside it. The setting is called **Show Performance Overlay** now and has a left/right placement; its
stored key `show-fps-counter` is unchanged, so no unit loses its value and no seeded settings file
needs editing. Nothing in it touches audio or the transport. A rig script that greps the old label is
the only thing that breaks.

**The Old Android commit carries a latch that can silently change what these runs measure, and it
is the one thing in section 3 that will bite.** `VideoStarvationPolicy` counts sessions that reach
the handshake and render no video frame at all, and **it is not gated by API level**. On the third in
a row `CommManager` writes `video-profile-starvation-cap`, which forces the next session to at most
1280x720, 30 fps **and AAC audio** over whatever Video settings say. Only the user changing the
resolution or the frame rate clears it: a session that renders deliberately does **not**, because
capping is what made it render. Three fumbled bring-ups during setup is an ordinary morning, so this
will latch on a unit nobody chose to latch it on, and it turns AAC **on**. H2 is the
default-configuration arm and expects `use-aac-audio=false`; D1b is supposed to be the arm that
forces AAC. **Read `video-profile-starvation-cap` back false before every run and report it after**,
exactly as for `playback-focus-self-defeating`, which cost Part G a whole round for the same reason.

**The rest of that commit is inert on D-HU and D-POCO and live on D-SAM.** The narrow-band profile
cap now fires below API 21, where neither band signal exists, and a platform that renames every P2P
group says so below API 29. D-HU is API 34 and D-POCO API 35, so neither arm runs there. D-SAM is
API 19 and both do, which is part of why S1 is worth taking.
...
phone, the P2P group that survives a force-stop, and the reinstall that wiped `settings.xml`. That
section now opens with a **unit table**, with each unit's serial and the rule that a quirk naming no
unit was measured on D-HU. Read it rather than this brief for anything about a unit. Two names to
keep straight while you do: **`D-T230` in the `t230-native-aa-bringup` rounds is D-SAM**, and the
Samsung phone in the `usb-device-diagnostics` rounds is a USB test peripheral, not a rig unit.
...
**The last two are latches rather than preferences**, so they are the two keys worth reading back
after a run as well as before one. A round that finds either set part-way through has measured a
different build state from that point on, and should say where.
...
**PASS:** `commit` reads `f2e8d5bf` on the candidate and `8418ed02` on the baseline, and the gate is
1974 tests with no failures. Report both APK md5s.

**Gate D-SAM separately if S1 runs.** It builds the same APK, so `commit` should read `f2e8d5bf`
there too, but the install path is `install_and_launch.sh` with `HU=30041c35642d2200` rather than the
usual one. Report its md5 beside the other two.
...
## Part S: the sink on Android 4.4, one run

D-SAM (Samsung SM-T230, API 19, serial `30041c35642d2200`) is new to the roster and this is the
first time any audio work has met it. **Every API gate in `AudioTrackWrapper` falls the other way
there.** `getUnderrunCount` and `getBufferCapacityInFrames` are API 24, so the underrun count comes
from the fallback that calls a buffer which has run dry while playing the same evidence, and the
capacity comes from the bytes we asked for rather than from the track. `getBufferSizeInFrames` is
API 23, so `effective=` falls back to the capacity. And `MediaCodec.Callback` is API 21, so AAC runs
through `decodeSync` rather than the async callback and the whole write-executor and semaphore path
this thread spent two rounds on is not the code that runs. None of that has ever executed on
hardware, on any build.

### S1. Does the instrument say anything at all on API 19

**Candidate only.** One session, about five minutes, media playing throughout, listening and marking
as in Part P. Default settings with `audio-latency-multiplier` **absent**, `use-aac-audio=false`.

Report every `audio sink AUDIO` window in full, plus the `AudioTrackWrapper: AUDIO can hold ...`
construction line and `playback started with N frames banked`.

**PASS:** the sink line prints at all, `capacity=` and `target=` are non-zero figures rather than
placeholders, `target=` reads 400 ms, and `underruns` moves at least once across a session in which
the listener heard something break.

**FAIL:** no `audio sink` line, `capacity=0`, or `target=1`. Any of those means the instrument this
whole thread is built on reports nothing on an old unit, which would matter well beyond this round.

**Report `underruns` against what you heard, plainly.** The API 19 underrun fallback is a different
mechanism from the framework counter every other run uses, so a session with audible breaks and
`underruns=0` here is a finding about the fallback, not the falsifier firing again.

**D-SAM's own constraints are in `TESTING-TEMPLATE.md` section 7a and all of them apply.** The four
that will stop the run if missed: there is no `su` and **no `sed`**, so a settings change is a
host-side edit of a copy of `shared_prefs/settings.xml`, a push, and a `run-as ... cp`, never
`set_hu_pref.sh`; the USB port is data-only and the battery has been sitting at 9%, so charge it
from a separate supply first or the round measures a browning-out radio; mount or hold it in native
portrait with `screen-orientation=2`; and cap video at 720p, because at 1080p the link takes the
socket down with `EPIPE` about 3.5 s in and never renders a frame. A build after 2026-09-16 caps it
without being asked, so read `[RES_CAP] ... linkCapped=` before setting anything by hand.

**S1 is the only D-SAM run this round, and a PASS is not coverage.** The API 19 audio path has a
synchronous AAC decoder, a different underrun fallback and no real capacity reading, and grading all
of that is its own round. S1 answers one question: whether the instrument produces numbers there.

---

## 8. Do not run

- **B2, B2b, C1, D1b, G1, G3, G4, G5, G6** all passed and nothing in the Audio or Transport commit
  touches their mechanism. **D1b is the exception to watch**: if `video-profile-starvation-cap` has
  latched, AAC is forced on every arm and D1b stops being a forced arm at all. Read the key first.
...
- **G7** stays unrun.
- **Anything on D-SAM beyond S1.** The API 19 sink has a synchronous AAC decoder, its own underrun
  fallback and no readable capacity, and grading those is its own round rather than an appendix to
  this one.
...
7. **S1:** whether the sink line prints on API 19 at all, and what `capacity=`, `effective=`,
   `target=` and `underruns` read there against what you heard.
8. **`video-profile-starvation-cap` and `playback-focus-self-defeating`**, read back after every
   session as well as before. If either turns up set part-way through, say which run it was, because
   everything after it measured a different build state.
