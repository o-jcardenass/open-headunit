# aap package reorganisation, round 2 brief

Thread: `aap-reorg`. Round 2. Round 1 (`aap-reorg-round1-results.md`) was R0 FAIL: the relocation
put six files into `connection.wifi.modes.native`, `native` is a Java reserved word, and kapt's
Java stub for `SettingsFragment` named `modes.native.CredentialField` in a signature, failing
`:app:kaptGithubDebugKotlin` before any test compiled. R1-R3 never ran. This round re-runs the
same four runs against the fixed branch.

## 1. Build and baseline

Candidate: branch `fix/head-unit-server-silence-and-log-attribution` at `6479a374`.

```bash
git fetch fork fix/head-unit-server-silence-and-log-attribution
git checkout 6479a374
```

**History was rewritten since round 1.** The relocation commit was amended in place so the fix is
inside it rather than on top: the whole `connection/wifi/modes/native/` package, the six moved
files and the nine that already lived there, is now `connection/wifi/modes/nativeaa/`, with the 47
referencing files updated. Old `65212776` is now `f824721c`, old `7395d21b` is now `6479a374`; the
tree difference between the old and new tips is exactly the 47-file rename, 69 lines, all package,
import or KDoc-link text. Round 1's SHAs are orphaned; do not check them out. There is no baseline APK and no second
...
got 780/0. The two commits under test add and remove no tests, so the expected count at `6479a374`
is exactly 780/0.

## 2. What this is and why it exists

`f824721c` relocates 118 files: everything in `aap/` that is not AAP protocol moved to
`decoder/video/`, `decoder/audio/`, `input/`, `connection/*`, `app/` and `utils/`, and every test
moved into its subject's package. The commit is relocation only, verified by review. `6479a374`
...
- R0's expected count is exactly 780, matching the on-record `58802778` run. Any delta means a
  moved test dropped out of discovery. The package rename adds and removes no tests either.
- Round 1 left the previous thread's APK (`3.3.0-beta2`, md5 `7d793c0c...`) stranded in
  `hur-wifi-test-scripts/apks/` because `build_hur.sh` deletes and copies only on success. Verify
  the md5 of a freshly built APK before installing anything; never trust `apks/` contents by name.
...
Verified with `grep -F` against `6479a374`; none of them lives in the renamed package.
...
Build `assembleGithubDebug` at `6479a374` and run `run_unit_tests.sh`. Record executed and failed
...
Every `Java_` symbol must decode to a class that exists in the source tree at `6479a374`
...
`f824721c`, and the tap traverses the touch mapper to the phone. No separate input run is needed.
...
Nothing from round 1 carries: it produced no APK and no run. The build-failure diagnosis itself is
settled; if R0 fails again it is a different failure, quote it fresh. The relocation's compile-time correctness is already
