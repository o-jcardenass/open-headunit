...
| Thread | State | Next |
|---|---|---|
| `wpp-endpoint-depoison` | **QUEUED, round 2** | `wpp-endpoint-depoison-round2-brief.md`, candidate `ffca5a1f`, 2175 tests. Round 1 scored R0/R1/R3 PASS with no FAIL; its R2/R4 INCONCLUSIVE was **not** a radio fault. The unit was carrying the access point's `static-bssid`, `WifiDirectManager` announced it as the WiFi Direct group's address, and the phone cannot join a name and address pair that does not exist - round 1's own captures carry `BSSID: 00:27:15:43:06:6A (source=static override)` beside a source dump naming the real address one rung below. `ffca5a1f` gives WiFi Direct its own setting and R6 grades it. R2 is also redesigned: round 1's version took the poisoned network down, so the phone failed at the network layer and never dialled at all. Round 2 leaves the access point up and moves only `native-ap-transport`, which is the one state that refuses a dial the phone can actually reach. |
