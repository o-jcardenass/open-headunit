...
| `release-next` | **queued** | round 1 done: #852 is fixed clean (R1 reproduced it 6x on `v.3.2.5`, R2 showed zero on the candidate, R3 confirmed recovery survived the split, idle cadence measured 679-758ms), R5/R7 passed, **R6 FAILED**. R6's own conclusion — a merge regression in `AapTransport.kt` — is wrong: those hunks are disjoint from the escalation path and the merge's one delta there is inert on the `SURFACE` backend R6 ran. The real defect is `fix/video-pipeline`'s and round 1 is the first run that could hit it: R6 injected **90** faults where video round 2's identical run injected **2**, so sustained loss had never been tested. A keyframe whose middle fragment was dropped still scans as a keyframe, and feeding it both disarmed the starvation branch and cancelled the escalation clock. Fixed, plus a fault budget so one capture can show the damage *and* the repair. The three feature branches are now **one linear branch, `fix/video-stack` @ `6911d3c5`, nine commits on `main`** (486 tests), byte-identical to round 1's merged tree up to the last two commits. **Round 2 is queued: `release-next-round2-brief.md`.** Do not re-test #852. |
