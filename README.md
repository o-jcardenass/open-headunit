...
| `native-aa-dsam-wifi-unavailable` | **FAIL** | native-aa-dsam-wifi-unavailable-round2-results.md. Part A: no field/API/config the app controls keeps a group alive on D-SAM. Part B: `5846b249` PASSes 5/6 hits (~3.1-3.7s reaction) but FAILs 1/6 — its detection is armed only incidentally, via a `refreshNativeCredentials()` refresh unrelated to the removal event, not the `WIFI_P2P_CONNECTION_CHANGED_ACTION` disconnect broadcast itself. |
