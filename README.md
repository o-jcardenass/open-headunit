...
| `media-gap-instrument` | **round 1 done** | R0 **PASS** (first-ever compile clean, 588/588 tests, both named regression tests pass). **R1 PASS** — the four zeroes, all zero, on a clean 10-minute session. **R2 FAIL — the headline finding**: `inbound video quiet` fired 4 times on a genuinely stationary Maps screen with no navigation, `dead=95-99%` per window; source review shows the `MIN_GAPS_MEDIA=2` floor does not suppress this because video keeps trickling in sparse isolated packets (periods 3.3-17.9s) rather than going fully silent — a real false-positive risk on exactly the scenario the round exists to rule out. **R3**: first attempt was inconclusive for the wrong reason (a stale `enable-audio-sink=false` inherited from a restored settings.xml meant the audio channel was never even negotiated); fixed and rerun, genuine **PASS** (audio quiet fired twice with gaps=2/3, link stayed quiet throughout, 6 `Media Sink Stop Request: AUDIO` lines matching 6 scripted pause cycles). **R4**: the brief's "no station association" premise doesn't hold on this rig (joined to a real 5GHz network since that morning) — required unjoined arm is **UNTESTABLE** without a hard-to-reverse `forget-network` this round didn't authorize; optional joined arm **PASSED** twice (correct combined W-level coexistence line, frequencies differ). **R5 INCONCLUSIVE** — only 30 fault candidates at 1-in-300 in 5 minutes, 0 injected is expected; no wedge, fps stayed healthy. See `media-gap-instrument-round1-results.md`. Nothing queued next for this thread. |
