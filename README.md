...
| Thread | State | Next |
|---|---|---|
| `wpp-endpoint-depoison` | **INCONCLUSIVE, round 1** | `wpp-endpoint-depoison-round1-results.md`, `020c6904`, 2170 tests. R0/R1/R3 PASS, no FAIL: the hotspot-transport endpoint advertise, reconnect-without-RFCOMM and no-regression-on-a-servable-dial all check out clean on Gearhead 17.8. R2/R4, the round's actual point (does the rejection clear an already-poisoned phone), are INCONCLUSIVE: this rig's WiFi Direct join fails outright right now regardless of the branch (`STATUS_WIFI_NETWORK_UNAVAILABLE`, phone-side `groupFormed: false` on every attempt), so no dial ever reached the rejection path either way. Side finding worth a follow-up: a plain network-unreachable failure alone made Gearhead 17.8 drop its own stored endpoint once, with no rejection message involved. Needs a round 2 once WiFi Direct join works on this rig, or a lever that keeps one network reachable throughout instead of switching transports. |
