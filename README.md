...
the same way. Asset filenames are unchanged, so fetch by filename rather than by the tag a results
file names:
...
| Thread | State | Next |
|---|---|---|
| `dpoco-export-logs-silent-failure` | **Findings only, needs fix** | `dpoco-export-logs-silent-failure-findings.md`, `d9e519259`. Export Logs silently returns null when `log-source=APPLOG_FILE` and Start→Stop happens with nothing logged in between: the empty capture file gets deleted on close, but `AppLog.lastLogFile` still points at it, so the exporter's `.exists()` check fails with zero diagnostics. Root-caused to `LogExporter.kt:352-355` / `AppLog.kt:104-106`; suggested fix is a ring-buffer fallback on that branch. Not yet fixed. |
| `usb-version-retry` | **Round 3 DONE, PR-ready** | `usb-version-retry-round3-results.md` (`181e7138`): no FAIL. R0/R1/R2/R4 PASS (R4 hit the literal `preempts` line 3/3, never seen in round 2); R3 not run (no 2nd phone). R5 step 1 UNTESTABLE (dongle retries ~60s apart, not continuously, so the 60s-budget line can't fire), step 3 PASS. |
| `hotspot-endpoint-poison` | **Round 3 DONE, no FAIL, PR-ready** | `hotspot-endpoint-poison-round3-results.md`, `29abed1773c0`, 2340 tests. R4 **PASS**: `24682388` fixes round 2's finding, the stale-endpoint counter (E) now stays non-zero across a second landing instead of self-clearing. R7 **PASS**: the WiFi Direct retire path (key reads back `0`) is unaffected. Rounds 1-3 together clear every run in this thread. |
| `wifi-direct-credentials` | **Round 2 DONE, no FAIL, PR-ready** | `wifi-direct-credentials-round2-results.md`, tested on `testing/native-aa-wireless-and-auto-start-loading` @ `b096460bf` (merges brief candidate `29ef268e`), 2306 tests. R2r **PASS**: with the round 1 group still live, a passphrase-only change now recreates the group (`matchesRequest=yes`, fresh BSSID) and `wifi-direct-readback-passphrase` reads the new value. C1 **PASS**: an unchanged pair still takes the survivor-read fast path, so the fix did not disable it outright. |
| `istartup-binder-probe` | **DONE, round 1** | `istartup-binder-probe-round1-results.md`, candidate `6699862` on **`o-jcardenass/wireless-helper`**, not this repo. P1-POCO **OPEN**, P1-MOTO **CLOSED** on the identical Gearhead `17.8.163804-release.daily` build, confirming per-device Phenotype cohorts disagree as expected. P2 deep arm on D-POCO (OPEN handset) came back **CLOSED on dispatch** ("Caller is not allowed") despite the OPEN descriptor query, so the descriptor-level check and the real transaction are two separate gates and only one is controlled by the named flag; the fallback-chain design should not treat D-POCO's OPEN as fully usable without accounting for that. |
| `native-aa-wireless` | **Round 4 DONE, no FAIL, PR-ready** | `native-aa-wireless-round4-results.md`, `181f8e16`, 2266 tests. R25 **PASS**: the adopt-vs-assess race stays closed even from a deliberately poisoned `STABLE` verdict (a genuine create landed first and overwrote it before the adopt cycle ran), no endpoint ever advertised, and a full session formed with `identity stable=no` throughout. R26 **PASS**: `headunit://exit` starting the app no longer lets the in-flight callback adopt on a stopped manager (`the adopt-or-create decision abandoned` fires, no `Group formed` follows); the group staying up afterward is expected, not a FAIL. R27 **PASS both halves**: ordinary exit still removes the network, 80ms (no session) vs. 1942ms (session live) stop-to-cleanup delay, matching round 3's 180/1972ms figures. **New non-candidate finding**: D-POCO's Gearhead cached a stale RFCOMM target (another rig headunit's real BT MAC) from unrelated earlier testing and needed an app-data clear to unstick, a third phone-side failure signature alongside round 3's two. Round 3 (`native-aa-wireless-round3-results.md`, `ec7c84f8`, 2264 tests): R22 **PASS**, baseline (`3e7237d3`) reproduces the self-comparison defect on D-SAM and the candidate closes the race, with a full session withholding the endpoint at `identity stable=no`; R23 **PASS** above the naming API on D-HU; R24 **PASS**, both halves tore the group down, refuting the brief's `ServiceStopWaitPolicy` explanation. **R20's divergence is now answered from round 2's own capture**: `headunit://exit` on a stopped app starts the service, whose `onCreate` arms the bring-up, so `stop()` runs before ownership is established, skips its removal, and the callback adopts 30 ms later on a stopped manager. The group surviving an exit in that state is a separate, still-open change. `native-aa-wireless-round2-results.md` for round 2, `-round1-results.md` for round 1 (17/17 no-FAIL). |
| `wpp-endpoint-depoison` | **Round 5 DONE, no FAIL, PR-ready** | `wpp-endpoint-depoison-round5-results.md`. `d49ce471f` (`8e6c27db` + the rig's `6b4eb0d9d`, rebased and pushed). R0-R3, R5-R7 PASS, R4 not reached. **R3 landed clean on the first try**: reads the surviving group, verdict `CHANGED` with no yardstick, refuses the phone's stale dial in 0.7s, phone drops the config and falls back to Bluetooth. R5's 10/10 reconnects (5 exit, 5 force-stop) all succeeded. Two errata: the brief's named phone-side instrument (`Won't persist Wifi Configuration...DYNAMIC`) does not exist on this Gearhead build (17.8 logs a generic, non-discriminating line instead; the WPP-dial line is what actually proves field 5), and its passphrase-key name is wrong (`wifi-direct-group-passphrase`, not `wifi-direct-passphrase`). New, out-of-brief finding: the `PHONE_HOLDS_STALE_ENDPOINT` home-screen banner can never clear itself once R3's own fix succeeds (it only clears on a served WPP dial, which a working fix makes unreachable) — confirmed stuck live after R5's 10 clean reconnects. |
...
| `hold-aa-rfcomm` | **DONE, round 1 addendum 3** | `hold-aa-rfcomm-round1-addendum3-results.md` on `c3c5a2d8`: A1-A4 (D-POCO, both bands, both hold positions) and B1-B2 (D-SAM, both hold positions) all HELD, no collapse anywhere; hands-free stayed down at every ring, so the stand-in-link-at-ring hypothesis is still untested. Addendum 1's original collapse was not reproduced under full instrumentation. A2-A4 ran on a swapped-in phone (Galaxy S24+), not D-MOTO as briefed. |
...
| `pause-auto-connect` | **DONE** | round 1 (`1984b1ad4`) no FAIL, see round1-results.md + addendum; its SSL leg PASSed as usb-version-retry round 1 R3. The before-SSL X case PASSed as usb-version-retry round 2 R6. |
