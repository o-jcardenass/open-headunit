...
| `hold-aa-rfcomm` | **QUEUED, round 1 addendum 3** | `hold-aa-rfcomm-round1-addendum3-brief.md` on `c3c5a2d8`: an incoming call against band, hold switch and a live stand-in link, D-POCO head unit then D-SAM. Round 1 (`hold-aa-rfcomm-round1-results.md`) no FAIL, retry suppressed. Addendum 2's calls had no Bluetooth link up at the ring (TCP-formed session, 5 GHz), so it does not narrow addendum 1's collapse. |
