...
| Thread | State | Next |
|---|---|---|
| `wpp-endpoint-depoison` | **DONE, round 2** | `wpp-endpoint-depoison-round2-results.md`, candidate `8b3e15f3`. R0/R1/R3/R6 PASS, R2 INCONCLUSIVE (this rig's WiFi HAL cannot hold a SoftAP and a WiFi Direct GO interface at once - platform tears the AP down, not the app - so the rejection code path was never reached), R4/R5 not reached (depend on R2). R6 confirms the BSSID setting split fix and that the phone now associates to this unit's WiFi Direct group at all, unlike round 1. |
