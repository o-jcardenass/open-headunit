...
| `link-stall-periodic-scan` | **round 5 done — round 4 still queued** | Round 5 tested whether a WiFi station scan blanks the P2P radio (MediaTek's own ~8.1s dual-band-scan constant on the reporters' silicon). **R0-R4 all PASS.** R0's unit-test gate found and fixed a real, pre-existing, scope-unrelated bug (`P2pOperatingChannelPolicy.frequencyMhzFor(14)` missing the 2484 MHz special case; one-line fix, local commit `307d85f2`, not yet pushed), then 552/552. **R1 (unassociated, the rig's default state): zero stalls > 1.2s over 733s** — this is the brief's own pre-registered "mechanism absent" outcome, not a failure — with scan events sparse and irregular (7 events, 27-160s gaps), one to two orders of magnitude sparser than MediaTek's 8.1s constant. **R2 (associated to 5GHz): also zero stalls**, but scan rate roughly halved vs R1 (3 events/665s vs 7/733s) — confirms the *directional* claim (unassociated scans harder) even with no stall difference to go with it, on hardware that cannot reproduce the stall on either arm. **R3 (`wifi_scan_always_available=0`): setting took, but scan rate was unchanged (if anything slightly higher, 9 events/674s)** — no suppressive effect on this stack, reported as the brief instructed rather than forced. **R4 (the only rig-testable check of `fix/wifi-direct-lifecycle`): both lifecycle fixes confirmed working** — re-arm after a clean user exit forms a full second session with no join-watchdog teardown against the joined group, and the manual poke button now re-arms closed listeners and successfully pokes (`Native AA listeners are closed — re-arming before the poke.` → `Successfully poked`), where before the fix it did nothing. Per the brief's own honest limit (§2): **none of this refutes the theory for the reporters' MediaTek silicon** — it confirms this UNISOC/Android-14 rig cannot carry #839/#824's fault at all, since its own scan cadence never comes close to the timescale the mechanism needs. See `link-stall-periodic-scan-round5-results.md`. **Round 4 (`link-stall-periodic-scan-round4-brief.md`, 2.4GHz band + constrained memory profile) remains queued**, now the weaker of the two remaining hypotheses. |
