...
| `native-aa-dsam-wifi-unavailable` | **NEW, unscripted, round 1** | native-aa-dsam-wifi-unavailable-round1-results.md, D-SAM app `3.5.0-alpha` (113). Two live Native AA connect attempts on D-SAM both showed the reconnect-loop symptom (4 and 2 group-create cycles respectively) resolving once the phone accepted the network; the retry is the phone's own `Type 6 WifiConnectStatus=WIFI_NETWORK_UNAVAILABLE(-11)` reply after a clean handshake, not an app-side group-teardown race — the `isHandoffSettling()` 45s guard was never challenged. Likely the same mechanism as `link-stall-periodic-scan`; not queued as a formal round of that thread. |
