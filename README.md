...
- `wpp-endpoint-depoison-round3-brief.md` (no access point anywhere in R1 to R3; the poison goes out over WiFi Direct, where the group owner address never moves)
...
| Thread | State | Next |
|---|---|---|
| `wpp-endpoint-depoison` | **PR-ready, round 3 queued** | `wpp-endpoint-depoison-round2-results.md` and its addendum, candidate `8b3e15f3`. Round 2: R0/R1/R3/R6 PASS, R2 INCONCLUSIVE (this rig's WiFi HAL cannot hold a SoftAP and a WiFi Direct GO interface at once, the platform tears the AP down rather than the app, so the rejection path was never reached), R4 desk check, R5 not reached. R6 confirms the BSSID setting split and that the phone now joins this unit's WiFi Direct group at all, unlike round 1. The addendum measures the other half: the phone re-dialled the dead endpoint for 9m24s without giving up, ran its Bluetooth handshake in parallel throughout, and kept the stale address through a handshake that withheld one. `wpp-endpoint-depoison-round3-brief.md` poisons over WiFi Direct instead, where the advertised address is the group owner's `192.168.49.1` and therefore survives every create, so no access point is needed and the HAL is never asked for two interfaces. |
