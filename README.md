...
| `link-stall-periodic-scan` | **redirected — round 3 brief queued (the codec A/B)** | The discovery-loop question is **answered** by `release-next` round 6: R4 the loop never arms on a plain native session (0 `Discovery active`), R5 it survives a deliberate mode-2→mode-3 switch exactly as source-predicted but **stops at the phone's WiFi join**, 8.4s before the SSL handshake, and never carried the session-connected suffix — 32 lines, 0 with it. It is gated on the P2P client landing on the WiFi network, which happens in both modes, not on `isClientConnected` staying false in native mode. A real bounded defect, but it cannot silence a live session, so **it is not what #839/#824 are** and the previously-queued "switch modes later" round is dropped. What replaced it: reading all five of #839's reporter captures against `Media Sink Setup Request: N on channel VIDEO` (the phone's own request, INFO-level, stable enum) shows the controlling variable is the **video codec** — every H.265 capture 0 audio underruns, every H.264 one 8–10/min, on one unit, one phone, across three app versions. Cause is `SystemOptimizer.kt:106` (`hasH265 && panelCeil.width > 1920`), which v.3.1.1:92 did not have. The unmeasured half is the wire cost: H.264 at 720p30 was only 1.08 Mbit/s in his VERBOSE capture, so "twice the bits" is unproven. `link-stall-periodic-scan-round3-brief.md` measures both codecs' bitrate on identical content. Note for whoever runs it: **this rig has already measured H.264 clean twice** (round 6 R4/R5, 0 stalls both), so the reporter's fault is not expected to reproduce here and the primary metric is bitrate, not stalls. |
