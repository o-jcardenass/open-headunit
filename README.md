...
| `audio-sink-jitter` | **QUEUED (round 3)** | round3-brief.md against `bd7eed16`, gate 1932/0. A repeated Media Sink Setup no longer destroys a live sink, the default cushion is 16 and a sink can deepen its own, and the video ack is the phone's flow control again. P5 is the headline; the round's real question is what `dumpsys media.audio_flinger` says at each marker. |
