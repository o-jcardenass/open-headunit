...
| `hold-aa-rfcomm` | **DONE, round 1** | `hold-aa-rfcomm-round1-results.md` on `c3c5a2d8`, no FAIL, retry fully suppressed. Addendum: D-SAM's stand-in HFP dies on an incoming call unless its own BT is off. Addendum 2 (`...-addendum-standin-hfp-bt-coexistence-2.md`): same stand-in path confirmed engaged on D-POCO as head unit, but the collapse does not reproduce there across 3 calls — narrows the cause toward D-SAM's specific radio hardware. |
