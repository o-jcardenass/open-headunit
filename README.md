...
| `media-gap-instrument` | **queued** | `media-gap-instrument-round1-brief.md` — first-ever compile of `fix/media-gap-instrument-and-attribution` @ `3398c8cc`. Three instruments watched a reporter's media-only outage and none reported it; this branch is those three. **The rig cannot carry that fault** (`link-stall-periodic-scan` round 5 settled it) so the round asks the other question: are the new lines silent on healthy hardware. R1's PASS condition is four zeroes. |
