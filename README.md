...
| `video-pipeline-stack` | **queued** | `video-pipeline-stack-round1-brief.md` — thirteen runs over **two builds** in one visit. Build A (`f008e3d1`) is a four-branch video stack, never compiled anywhere, so R0 is a hard gate; build B (`becebffa`) is one transport fix whose only rig question is a soak. R1 harvests the SPS diagnostic that decides a dependency question outright, R3-R5 use the new hidden fault-injection setting to reproduce the artifact reports on healthy hardware, and R10-R11 carry the backpressure and capability lines added after a Galaxy Tab S7 FE's logs. |
