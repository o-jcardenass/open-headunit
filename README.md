...
| `release-next` | **answered, pending a decision** | round 1: #852 fixed clean, R5/R7 passed, R6 FAILED for the wrong reason (a merge regression it blamed on `AapTransport.kt`, disproven) but found a real defect: a keyframe with a dropped middle fragment still scans as one, disarming the starvation branch and cancelling the escalation clock. Fixed on `fix/video-stack` @ `6911d3c5` (nine commits, 486 tests). **Round 2 done: R0/R3/R4/R5 PASS, R2 PASS (recovered 63.1s after the fault budget was spent), R1 FAIL** against the brief's literal bar (the `but restart suppressed` ladder returns, three times) but with a load-bearing qualification — cycle 1 of the escalation fires in ~2s exactly as the fix intends every time, and what recurs on cycles 2+ under continuous unbounded loss is a *different*, pre-existing mechanism: `KeyframeCycleEscalationPolicy.CYCLE_COOLDOWN_MS = 60_000L` (untouched by this round, guards against #755) forces a 60s minimum gap between cycles, and the independent `VideoDecoder` sync-stall watchdog's own restart-suppression ladder keeps thrashing during that gap. R2's 63.1s recovery gap is the same cooldown, not the escalation failing — cycle 1 had already fired before the fault budget was spent, so cycle 2 was waiting out its mandatory spacing. Also: R4 (zero phone interaction) reproduced `parameter sets changed mid-session` twice, overturning round 1's attribution of that to map panning. See `release-next-round2-results.md`. **Next: someone decides whether the brief's multi-cycle PASS bar needs revising, or whether the sync-stall watchdog's ladder during the cooldown gap is itself worth fixing.** Do not re-test #852. |
