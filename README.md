...
| `release-next` | **mostly answered, one follow-up worth queuing** | Round 5 confirms the mode-5 TLS-desync fix: **R1 PASS** on condition 0 (session survived a full 30-fault budget, zero session deaths), the hold fired for the first time ever (`holding the cycle` = 1), and R1 vs R3 shows the reader stage seeing what the assembler stage structurally cannot (17/40/1 vs 0/0/0). **R1's recovery gap (44.5s) beat R3's control this round but missed the ≤10s target**, and the actual recovery came from the decoder's own `sync_stall` ladder, not the new escalation (already spent 3.5 min earlier) — a real nuance, not a re-run away from resolving. **R3's 253.9s recovery gap is confounded**: budget-spend took 1029s this round vs round 4's 393s for identical settings, giving the `sync_stall` ladder 14 extra minutes to grind that round 4's R3 never had; not directly comparable, and would need a re-run that reaches budget-spent in the usual ~6-7 min to get a clean number. **R4 PASS, cleanly, all four reopens** (850-873ms escalation, ~1.5-1.6s repair, zero silence, zero alternation) — the #852 relaunch-onto-idle-screen fix holds up. **R5 PASS on a clean redo** (five zeroes, one `Codec initialized:`) after a first attempt was discarded for a device-interaction-inside-the-window setup error. **Worth a dedicated follow-up round**: that discarded R5 attempt showed the video pipeline going 123s with zero rendered frames from a fresh, untouched connect, self-corrected by the same warm-relaunch fix R4 exercises deliberately — not reproduced on the clean re-run, so not happening every time, but worth checking what fraction of connects hit it. See `release-next-round5-results.md`. |
