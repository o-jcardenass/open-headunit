...
| Thread | State | Next |
|---|---|---|
| `native-aa-5288` | **queued** | `native-aa-5288-round1-brief.md`. New thread. The stack was compacted again on 2026-08-20 into one ref, `fix/video-and-wireless-stack` @ `e9f5d2b6`, **565 tests**, and two of its five commits have never been compiled anywhere, so R0 is a real gate. The fault: the head unit forms its group, wakes the phone, hands out credentials, then aborts every handshake with `nothing is listening on port 5288` about every 4 s indefinitely, because `startWirelessServer()` read "assigned" as "running" and two of its four outcomes printed nothing at all. Four measured runs; R2 is the point of the round and R3 is the one whose failure would be worse than the bug. The manual poke re-arm and the lifecycle fixes are explicitly not re-run: round 5's R4 settled both. Round 5's channel-14 fix (`307d85f2`) is folded into this branch. Note for the other thread: `link-stall-periodic-scan` round 4 pins `c2983fc7`, from before the compaction, and now builds this same branch. |
