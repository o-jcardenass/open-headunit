...
| `video-pipeline-stack` | round 2 done | R0-R6 all ran; **R0 PASS** (454/454), **R2 PASS** — the fix works: both injected faults recovered in ~0.55s via `cycling video focus` → `keyframe reached the codec`, `rendered=` never hit 0 (round 1: 90+s), `Codec initialized:` only 3 vs round 1's 7-in-99s. **R3 INCONCLUSIVE** (0 faults in 5 min at rate=20, predicted by the brief). **R4 PASS** — the HIDE_START_CODE control stayed clean, `Codec initialized:` exactly 1 across 10 corrupted frames, zero starvation/focus-cycle lines. **R5** (mode-4 probe, no FAIL applicable): the probe is as inert as the already-known nudge — zero sink stops, zero new sessions, no attributable keyframe across 3 probe cycles — and because it replaces the session's only working lever, the run reproduced round 1's original wedge signature at full severity (`rendered=0` for 108+s, still unrecovered when the capture ended). **R6 PASS** — 10 clean minutes, zero `DELTA_CHANGED` (round 1: 10 false positives; audit fix holds), `Codec initialized:` exactly once, and `PARAMETER_SETS_ONLY` access units confirmed reaching this component for the first time, as the brief flagged. No crashes in any of the five captures. See `video-pipeline-stack-round2-results.md`. Nothing else queued — the fix is validated; ships whenever the PR is ready. |
