...
| `hold-aa-rfcomm` | **QUEUED, round 1** | `hold-aa-rfcomm-round1-brief.md` on `0b5b0170`. The Native AA Bluetooth channel is held for the session and the phone's pings answered, instead of closed at handoff. Measures whether the phone stops its RFCOMM re-dial loop while a hands-free link is up, and that exit, reconnect and a Bluetooth drop still work. The switch that turns the hold off is R6. |
