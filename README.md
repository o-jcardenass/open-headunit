...
| `release-next` | **queued** | Round 3 ran the focus-cycle hold on hardware and it never fired: **zero `holding the cycle until it settles` lines in all four runs**, recovery 66.9s against round 2's 63.1s. The tester's root cause is confirmed from source and is the round's whole value: `VideoFragmentAssembler` has no anomaly case for a dropped middle fragment, and `AapVideo.requestKeyframe()` - the only setter of the corruption clock the hold reads - is gated entirely on those cases. So the hold's trigger was structurally unreachable for the fault the thread exists for. Meanwhile `FragmentedMessageAudit`, written for exactly that fault, computed `DELTA_CHANGED` and only logged it. Fixed on `fix/video-stack` @ `bbf328e8` (one commit, 505 tests): `AuditRecoveryPolicy` routes `DELTA_CHANGED` on VIDEO into the existing keyframe request, and a new injector stage (`DROP_MIDDLE_FRAGMENT_IN_READER`, mode 5) drops the fragment *before* the audit counts it - the old mode 2 drops it after, which is why no earlier round could exercise this. R0/R3/R4/R5 of round 3 all passed and stand. **Round 4 brief queued: `release-next-round4-brief.md`** - R1 is the four-link chain (DELTA_CHANGED -> keyframe request -> hold line -> recovery under 10s) and R3 is mode 2 as the control that makes R1 mean something. Grep the bare token `FAULT INJECTED`, not the `AapVideo:` prefix - reader-stage faults print under `AapRead:`. See `release-next-round3-results.md`. |
