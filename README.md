...
| `release-next` | **queued** | `release-next-round6-brief.md` — **two threads on one candidate.** R1-R3 test the reserved focus cycle: round 5 showed all three cycles spent 207s before the loss stopped, so the last one is now held for a quiet wire and the quiet window is sized against measured fault spacing rather than our own report throttle. R4-R5 belong to the `link-stall-periodic-scan` thread below. Round 5's findings: `release-next-round5-results.md`. |
...
| `link-stall-periodic-scan` | **revived, running inside `release-next` round 6** | the suspect is now source-verified — `discoveryRunnable` is armed only by mode 2 / strategy 1 and never cancelled by a later switch to Native AA, so `discoverPeers()` takes the group owner off-channel every 10s for the whole session — and both the loop and the link-gap measurement now log at INFO. R4-R5 of `release-next-round6-brief.md` are the reproduction attempt. |
