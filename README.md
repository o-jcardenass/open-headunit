...
| `release-next` | **answered, pending a decision** | Round 4 (`fix/video-stack` @ `bbf328e8`, 505 tests) found the new reader-stage fault mode (`DROP_MIDDLE_FRAGMENT_IN_READER`, mode 5) **desyncs the TLS session and kills the connection within seconds of engaging, every time** - four sessions died 1-13 faults in, in one capture. Root cause confirmed from source: `AapReadSingleMessage.doRead()` drops the fragment's already-consumed bytes *before* handing them to `sslEngine.unwrap()`, so the phone's send-side TLS sequence number advances while ours does not - every later `unwrap()` fails record auth, cascading into `Connection closed (EOF)`. The code's own comment ("the stream stays framed") is true for TCP byte alignment and false for the TLS layer; R5 confirms this precisely - zero framing-desync lines fired even through the crash loop. **R1 and R2 are UNTESTABLE as a result** - no amount of retrying produces the sustained multi-minute mode-5 session either needs. What *did* work: `DELTA_CHANGED on VIDEO` fired 14 times and the new `fragment run lost bytes` recovery line fired 5 times in R1's short-lived sessions, so the detection-to-recovery wiring itself is confirmed live; only the sustained-session measurements (the hold, the budget, the <10s recovery gap) stayed unmeasurable. **R3 (mode 2, the control) and R4 (clean, 10 min) both PASS** - zero `DELTA_CHANGED`/`fragment run lost bytes`/`holding the cycle` on a clean session, third round running. **Next: `VideoFaultInjector`'s `Stage.READER` mode needs to feed the dropped message through `sslEngine.unwrap()` and discard the *output* rather than skipping the *input*, or no reader-stage fault mode can ever produce a testable session.** See `release-next-round4-results.md`. |
