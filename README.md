...
| `media-gap-instrument` | **round 2 queued** | Round 1: R0 **PASS** (first-ever compile, 588/588), R1 **PASS** (four zeroes, 45-55 fps), R3 **PASS** after a stale `enable-audio-sink=false` was found and fixed, R4 joined arm **PASS** (the required unjoined arm is UNTESTABLE here — the rig has been associated to a real network all along), R5 INCONCLUSIVE at 1-in-300 with no regression. **R2 FAIL was the round, and it changed the design**: a stationary Maps screen trickles rather than going silent, so 2-5 gaps landed in every window at `dead=95-99%` and the recurrence floor suppressed none of it. The media series now also require a window to be no more than 85% dead, and your capture is replayed as a unit test at its measured numbers. R4 also retired the last prescription from the coexistence line. See `media-gap-instrument-round1-results.md`. Next: `media-gap-instrument-round2-brief.md` @ `93354419` — short round, R2 re-run is the point. |
