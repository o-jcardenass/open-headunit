...
| `release-next` | **queued** | `release-next-round1-brief.md` — the first round on the integration branch `release/next` (`e2e65855`), which is `main` + `fix/video-pipeline` + `fix/session-liveness` and has **never been compiled**. Two questions, kept separate in the report: an A/B against `v.3.2.5` for **issue #852** (the paused full-screen music player covered by "Connection lost" — R1 must reproduce it on the baseline or R2 means nothing), and whether merging the two feature branches broke anything already measured (R5 and R6 re-check video round 2's own numbers on this rig; R7 checks the read-desync fix stays silent on a healthy link). Eight runs, two of which cost no rig time. Supersedes the `idle-screen-reconnect-overlay` thread — that brief tested #852 alone off a tag, and this one tests it inside the release candidate. |
