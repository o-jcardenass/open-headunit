...
| `release-next` | **queued** | Round 2 confirmed the holed-keyframe fix on its first cycle (2001ms against round 1 R6's 62003ms), zero false repairs across 53 discarded frames, 6 genuine repairs on a clean session, 486/486. It also measured the next defect without naming it: R2's picture took **63.1s** to return after the loss stopped, of which the lever itself was **0.96s** - cycle 1 had been spent 8.1s *before* the wire went clean, on a stream still breaking, and `CYCLE_COOLDOWN_MS` then held off the cycle that worked for a full minute with two cycles unspent. Fixed on `fix/video-stack` @ `e1c00ec7` (one commit, 495 tests): the transport now stamps when the wire last lost a frame and the escalation holds its cycle until it has been quiet for 2s, bounded at 60s so a wire that never settles still reaches the lever. The isolated dropped frame - the fault the escalation exists for - is provably unchanged, pinned by a test. **Round 2's attribution of the returning `restart suppressed` ladder to `CYCLE_COOLDOWN_MS` is wrong**: it is `DecoderStallCausePolicy.KEYFRAME_STARVATION_PATIENCE_MS = 15_000`, untouched here and out of scope, so the ladder is expected again in round 3 R2 and must not be scored as a regression. **Round 3 brief queued: `release-next-round3-brief.md`.** Do not re-test #852 or the holed-keyframe fix itself. See `release-next-round2-results.md`. |
