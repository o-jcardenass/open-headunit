...
| `release-next` | **queued** | Round 4 could not answer its own question: mode 5 killed the session within seconds of its first fault, four times in one capture (dying after 1, 13, 1 and 4 faults), so R1/R2 were UNTESTABLE. The tester's diagnosis was right and is confirmed from source - the drop discarded the message before `AapMessageIncoming.decrypt`, and `SSLEngine.unwrap`'s TLS record sequence advances per record, so a record never unwrapped desynchronises the session for good. The byte framing stayed intact throughout, which is why R5 saw nothing; the failure was one layer up. Round 4's R3 (61.6s recovery on mode 2) and R4 (three clean-session zeroes, now settled across three rounds) both passed. Fixed on `fix/video-stack` @ `64033c4a` (three commits, 513 tests): both readers now skip only `auditFragment` and the handler and decrypt unconditionally; the renderer banner names which of its four callers armed it; and a separate fix lands for the #852 reporter's follow-up, where a relaunch onto an idle screen goes black because the nudge loop was gated on the overlay and the escalation on video bytes. **Round 5 brief queued: `release-next-round5-brief.md`** - R1-R3 finish round 4's question, R4-R5 test the relaunch fix on TextureView. R4 needs four reopens because the bug alternates. Round 4's clean-session run is dropped as settled. See `release-next-round4-results.md`. |
