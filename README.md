...
| `idle-screen-reconnect-overlay` | **queued** | **Build from tag `idle-screen-round1-candidate` (`eec75197`), not from a branch** - the branch was folded into `fix/session-liveness` and deleted, and that branch's tip carries a second fix this A/B must not have. Brief updated accordingly. `idle-screen-reconnect-overlay-round1-brief.md` — issue #852: 3.2.5 covers the projection with "Connection lost" whenever the projected screen stops animating, because a watchdog that was dead until 3.2.5 measures frames and calls their absence a disconnect. Needs an A/B against a `v.3.2.5` baseline; R1 reproducing the defect is the gate for the whole round. **Pre-confirmed as reproducible on this rig**, unprompted, during `video-pipeline-stack` round 1's R12 (build B, unrelated to this defect): `showReconnectingOverlay` fired 4 times in one 10-minute capture, each cycle 6-16s, matching a live user report of "~10 seconds." That round's own gate (R1 reproducing the defect) should be a formality now — see `video-pipeline-stack-round1-results.md`'s R12 section for the log lines. |
