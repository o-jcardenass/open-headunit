...
| `video-pipeline-stack` | **round 2 queued** | `video-pipeline-stack-round2-brief.md` — tests the fix for round 1's R4 FAIL, the one run that blocked the stack from shipping. Round 1 found `DROP_MIDDLE_FRAGMENT` wedging the decoder into a permanent black-screen stall/restart loop (`rendered=0` for 90+ s, restart budget gone in 33 s, Android Auto raising its own "Switch Renderer" dialog). It is **not** a stack regression: the machinery is byte-identical to `main` and the loop is a keyframe-supply failure — a rebuilt codec resumes on a P-frame, AAP has no keyframe request, and every rebuild cancelled the clock that would have spent the one focus cycle that works. One build (`8f0beab1`), no baseline. Six runs: R2 is R4 re-run verbatim and is the point of the round; R4 is the HIDE_START_CODE control that must stay clean, and a failure there is worse than R2 failing. Three measurements ride along for free — whether the fixed fragment audit stops crying wolf (round 1: 10 false `DELTA_CHANGED` per session, then silent for the rest of it), whether Android Auto ever changes its parameter sets mid-session, and what `VIDEO_FOCUS_PROJECTED_NO_INPUT_FOCUS` actually does — a focus mode declared in Google's own protobufs that nothing has ever sent. Round 1's results stay valid as the comparison: see `video-pipeline-stack-round1-results.md` and `evidence/r4-decoder-wedged.png`. |
