...
| `video-pipeline-stack` | **round 1 done** | Build A: R0 PASS (422/422 tests). R1/R2 PASS — H.264 `num_ref_frames=1, num_reorder_frames=0` settles the SPS-rewrite dependency question against adopting it; H.265 clean too. R3/R5a/R5b (three of four fault-injection modes) PASS. **R4 (DROP_MIDDLE_FRAGMENT) is a FAIL**: the decoder wedges into a permanent black-screen stall/restart loop instead of degrading to a corrupt-but-live picture — Android Auto's own client surfaced a "Switch Renderer" dialog. R6/R7/R9 PASS. R8 PASS on the brief's named criterion, but is the round's second-biggest finding: an early stall/rebuild cluster tied to heavy-motion CPU load, and **two more live, unprompted #219 reproductions** on the bundled software HEVC path. R10 INCONCLUSIVE (rig can't manufacture codec pressure, matches round 6). R11 clean both codecs. Build B: R12 PASS, zero spurious disconnects. **R2 + R8 together are the strongest #219 evidence yet**: two structurally different decoders, both showing the same corruption class with byte-perfect transport and zero diagnostic signal — settles that the transport/reassembly layer is not the cause. **Build A is not ship-safe as-is** (R4's regression); build B is clean and independent. See `video-pipeline-stack-round1-results.md`; screenshots in `evidence/r2-*`, `evidence/r4-*`, `evidence/r8-*`. Nothing else queued for this thread; R4's root cause is the natural next round. |
