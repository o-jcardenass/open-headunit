...
| `link-stall-periodic-scan` | **rounds 4 and 5 queued — run 5 first** | The discovery-loop question was answered by `release-next` round 6 (see prior rounds for detail) and is not what #839/#824 are. Round 3 measured both codecs' wire cost on identical content, as queued: **R0-R4 all PASS**. R0: 525 tests, real hardware HEVC (`c2.unisoc.hevc.decoder`). R1 (H.265) and R2 (H.264, redone after the first attempt's Spotify silently paused mid-run — see results Setup notes) both **0 stalls, 0 underruns, ~100% audio delivery**, exactly as predicted for this rig. **Bitrate ratio R2/R1: 1.16× Mbit/s, 1.18× bytes/frame** — real but modest, landing in the gap between the brief's two pre-registered outcomes (not ≥1.4×, not within ~10%), so the bandwidth story alone doesn't explain #839's 8-10/min underrun rate. R3 desk check confirmed all three source readings on the pinned SHA: this rig's own panel would persist H.264 on first-time setup (`panelCeil.width=1920`, exactly at the `>1920` threshold), `isHevcReliable()`'s chipset allowlist rejects this rig despite real working HEVC (second data point against the predicate, matching #839's MediaTek unit), and confirmed **a real bug** — `settings.videoCodec=="H.265"` is decoder-side a command (`VideoDecoder.kt:750-761` discards `detectCodecType()`), not just an announcement preference — already fixed one commit past the pinned SHA (`b57eae3a` on `fork/fix/video-stack`, not yet tested here). R4 (optional, v.3.1.1 downgrade) built clean on the first try (brief expected friction) and found **no residual version effect**: 0 stalls/underruns, bitrate within the same ~12% noise band as R1/R2. See `link-stall-periodic-scan-round3-results.md`. **Round 4 is queued: `link-stall-periodic-scan-round4-brief.md`.** R3iii's bug is fixed and JVM-tested on the candidate, so it needs no hardware round. What round 4 does instead is put the rig on the two things it has never shared with the reporters' units — a **2.4 GHz** WiFi Direct group (a new debug lever; the rig could not run that band before) and the **constrained memory profile**, which has never run on hardware. It tries to *reproduce* rather than to measure, and §5 pre-registers what a clean result means: that this rig cannot carry #839 at all. **Round 5 (`link-stall-periodic-scan-round5-brief.md`) is queued ahead of it and should be run first.** It tests the first mechanism proposed on this thread whose timescale actually matches the fault: a WiFi station scan blanking the single radio. MediaTek's own Gen3 driver states a dual-band scan takes **8.1 s** on the silicon both reporters run, which brackets both waveforms (1.6 s/11.6 s and 5-6 s/10-11 s); MCC schedules against the beacon interval (~100 ms) and BT coexistence arbitrates at sub-ms to ~16 ms, so neither can produce seconds of silence. The rig's own station state has never been recorded in any round of this thread. Note the honest limit, stated in the brief's §2: the 8.1 s constant is MediaTek's and this rig is UNISOC on Android 14, so **a clean result cannot refute the theory for the reporters' units** — it only narrows it. Round 5 also carries the only rig-testable check of the `fix/wifi-direct-lifecycle` fixes. |
