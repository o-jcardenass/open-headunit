...
| `release-next` | **blocked** | round 1 done, **R6 FAILED**: #852 is fixed clean (R1 reproduced it 6x on `v.3.2.5`, R2 showed zero on the candidate, R3 confirmed recovery survived the split, R2's idle cadence measured 679-758ms), and R5/R7 passed — but R6 reproduced the pre-fix decoder-wedge signature (stall ladder exhausted twice, a 62s-late escalation, ~90s+ of `rendered=0`) on the exact fault video round 2 measured recovering in ~0.55s. The stuck recovery path, `AapTransport.escalateIfStillUnrepaired`, lives in `AapTransport.kt`, one of the four files auto-merged with no conflict between `fix/video-pipeline` and `fix/session-liveness`. **Blocks the release candidate** until that interaction is found and fixed — needs a dedicated bisection round isolating the auto-merged diff in `AapTransport.kt`. See `release-next-round1-results.md`. |
