...
| `link-stall-periodic-scan` | **R4-R5 run inside `release-next` round 6 — refined, not confirmed as session-long** | R4 (plain native session): the loop never arms at all (0 `Discovery active` lines) — confirms the leak needs the mode-2→mode-3 transition, not just any native session. R5 (the transition): the loop survives the mode switch unbroken exactly as predicted, but stops on its own ~8s before the phone's WiFi join completes and never carried the session-connected suffix — it looks gated on the P2P client actually landing on the WiFi network (true in both modes), not on `isClientConnected` staying permanently false in native mode as originally reasoned. No link silencing observed this run; `recv_gaps.py` and `LinkGapMonitor` agree at 0 stalls. See `release-next-round6-results.md`. Next: a round where the mode-2→mode-3 switch happens deliberately later (loop already ticking for minutes) to see whether the window before WiFi-join can overlap a live session on a slower join. |
