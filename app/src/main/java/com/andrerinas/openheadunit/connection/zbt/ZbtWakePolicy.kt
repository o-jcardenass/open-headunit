...
object ZbtWakePolicy {
     ...
     *
     * The three state flags are the same ones [com.andrerinas.openheadunit.aap.NativeHandoffPolicy]
     * applies to the ordinary poke, and for the same reasons: nothing should disturb the link while
     * credentials are being exchanged, while a handoff is settling, or once a session is up and
    ...
    fun shouldSend(
        ...
        if (settling || handshakeInFlight || sessionConnected) return false
