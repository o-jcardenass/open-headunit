...
class NativeAaHandshakeManager(
            ...
            try {
                ...
                while (isRunning && isActive) {
                    ...
                    if (socket != null) {
                        ...
                        val link = BluetoothSocketLink(socket, localRadioName)
    ...
    private suspend fun handleHandshake(link: HandshakeLink) = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            // one nothing in android.bluetooth can dial, so storing it would feed a wake path that
            // cannot work and a receiver that will never fire for it.
            val macs = settings.autoStartBluetoothDeviceMacs
            if (link.persistPeerForAutoStart && peerAddress != null && !macs.contains(peerAddress)) {
                AppLog.i("NativeAA: Saving $peerAddress ($peerName) to the list of auto-start devices.")
                val newMacs = macs + peerAddress
                settings.autoStartBluetoothDeviceMacs = newMacs
                settings.autoStartBluetoothDeviceName = peerName ?: "Unknown Device"
            ...
            suspend fun tick(budgetMs: Long) {
                if (readerClosed) {
                    ...
                    if (msg != null) {
                        ...
                        ifOwner(link) { resetHandshakeBackoff() }
