 ...
 * park is still happening rather than gone. It is bounded by the phone's own ack window, so a
 * `videoQueue` that climbs window after window means the ack is going out ahead of the work, and
 * `videoShed` above zero means even the transport's backstop ceiling was reached.
 *
