...
> **Revised 2026-09-23, before any run.** Every action on the app is an `AutomationReceiver` verb
> (template §3, `send ...`): stamp each arm with `send ACTION_LOG_MARKER --es text <arm>`, and
> never `input tap` the app. This brief has exactly
> two hand steps and both are named because no verb can exist for them: H3's banner **dismiss**
> (a UI control with no command behind it) and L6's WiFi toggle, which is D-HP's own system
> Settings rather than the app, on a unit where every scripted radio lever is documented closed
> (§7a). Anything else you find yourself tapping is a brief bug: stop and note it in Setup notes.
